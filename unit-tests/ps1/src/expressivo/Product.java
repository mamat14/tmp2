...
public class Product extends Expression {
    ...
    public Expression differentiate(String variable) {
        return new Sum(new Product(ex1.differentiate(variable), ex2), new Product(ex1, ex2.differentiate(variable)));
    }
    ...
    public Expression simplify(Map<String, Double> environment) {
        ...
        if (ex1Simp instanceof Decimal && ex2Simp instanceof Decimal) {
            return new Decimal(((Decimal) ex1Simp).number.multiply(((Decimal) ex2Simp).number));

        } else if (ex1Simp instanceof MyInteger && ex2Simp instanceof Decimal) {
            return new Decimal((new BigDecimal(((MyInteger) ex1Simp).number).multiply(((Decimal) ex2Simp).number)));
        } else if (ex1Simp instanceof Decimal && ex2Simp instanceof MyInteger) {
            return new Decimal(((Decimal) ex1Simp).number.multiply(new BigDecimal(((MyInteger) ex2Simp).number)));
        } else if (ex1Simp instanceof MyInteger && ex2Simp instanceof MyInteger) {
            return new Decimal(new BigDecimal(((MyInteger) ex1Simp).number).multiply(new BigDecimal(((MyInteger) ex2Simp).number)));
        } else {
            return new Product(ex1Simp, ex2Simp);
        }
    }
}
